...
namespace JomMalaysia.Framework.WebServices
{
    ...
    {
        IWebServiceResponse<T> ExecuteRequest<T>(string url, Method method, string auth, params object[] objects)
            where T : new();

        IWebServiceResponse ExecuteRequest(string url, Method method, string auth, params object[] objects);

        Task<IWebServiceResponse<T>> ExecuteRequestAsync<T>(string url, Method method, string auth,
